...
# 航向角设定
# set direction of the front of the Copter (latitude, longitude)
...
# 这些方法只是近似值，在较远的距离和接近地球两极时，可能不太准确。
...
# 设置车辆的目标位置（WGS84 坐标）、速度、航向或转弯速率。这与 SET_POSITION_TARGET_LOCAL_NED 消息类似（见上文），不同之处在于位置以纬度和经度值的形式提供，并且海拔高度可以高于海平面、相对于家庭或相对于地形。
def goto_position_target_global_int(aLocation):
    ...
    msg = vehicle.message_factory.set_position_target_global_int_encode(
        ...
        0b010111111000,  # type_mask (only speeds enabled)
...
# 设置车辆的目标位置（作为 NED 中距 EKF 原点的偏移量）、速度、加速度、航向或转弯速率。消息定义可以在这里找到
# type_mask已修改
def goto_position_target_local_ned(north, east, down):
    ...
    msg = vehicle.message_factory.set_position_target_local_ned_encode(
        ...
        0b010111111000,  # type_mask (only positions enabled)
...
# 速度控制函数
# 简单来说这里依然用的是老的mavlink协议进行通讯，区别不过是掩码不一致，使得我们可以直接操作他们的速度罢了，可以考虑结合位置代码完成更精细化的指令
"""
...
def calculate_absolute_target(current_angle, target_x, target_y):
    # 将角度转换为弧度
    angle_rad = math.radians(current_angle)
    
    # 计算目标点在飞行器朝向下的相对坐标
    relative_target_x = target_x * math.cos(angle_rad) - target_y * math.sin(angle_rad)
    relative_target_y = target_x * math.sin(angle_rad) + target_y * math.cos(angle_rad)
    
    # 计算绝对目标点的坐标
    absolute_target_x = relative_target_x
    absolute_target_y = relative_target_y
    
    return absolute_target_x, absolute_target_y
